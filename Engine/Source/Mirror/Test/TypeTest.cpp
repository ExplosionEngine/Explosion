...
{
    ASSERT_TRUE((std::is_same_v<Mirror::GlobalFunctionTraits<decltype(Add)>::RetType, int>));
    ASSERT_TRUE((std::is_same_v<Mirror::GlobalFunctionTraits<decltype(Add)>::ArgsTupleType , std::tuple<int, int>>));
    ...
    ASSERT_TRUE((std::is_same_v<Mirror::MemberFunctionTraits<decltype(&TestClass::Add)>::ClassType , TestClass>));
